...
// Online player state: playerId → { username, x, y, dir, socketId }
const onlinePlayers = new Map<string, { username: string; x: number; y: number; dir: number; socketId: string }>();
...
export function registerSocketHandlers(io: SocketServer, deps: Deps) {
  ...
  io.on('connection', (socket: Socket) => {
    ...
    socket.on('player:join', async (data: { playerId: string; username: string; x?: number; y?: number }) => {
      ...
      // Store in online players map (default position near spawn)
      ...
      // Send the full list of online players to the joining player
      const others = [...onlinePlayers.entries()]
        .filter(([pid]) => pid !== data.playerId)
      ...
      console.log(`[Socket] Player joined: ${data.username} (${data.playerId}). Online: ${onlinePlayers.size}`);
    });
    ...
    socket.on('player:move', (data: { x: number; y: number; dir: number }) => {
      ...
      const playerState = onlinePlayers.get(playerId);
      if (playerState) {
      ...
      // Broadcast to everyone else with username so client can render them
    ...
    socket.on('disconnect', () => {
      ...
      if (playerId) {
        ...
        console.log(`[Socket] Player disconnected: ${playerId}. Online: ${onlinePlayers.size}`);
      }
    });
  });
}
